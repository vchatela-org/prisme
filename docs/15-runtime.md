...
PostgreSQL, the identity provider present and already serving forward-auth for another application.

Two corrections to an earlier reading of the cluster, both measured:

- **Ingress is the proxy's own route CRD, not Gateway API.** Gateway API types are installed — k3s
