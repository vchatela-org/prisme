...
| `AREA_COLOR_PINS` must be set by hand and **nothing generates it**, so an instance that pins nothing keeps the collision — `areaColorCollisions` can report the clash and only the gallery calls it | FUP-2026-09-21-area-colour-pinning | 🟢 the **Areas** screen names the colliding areas and prints the line to set, keeping what is already pinned and dealing only the gaps — proved in a browser both with the variable unset and with the proposed map applied | [#75](https://github.com/vchatela-org/prisme/pull/75) |
...
| The deployment's drift alert was **removed rather than repaired**, and `last_drift_full` is recorded with no reader; `apps/sync/src/main.ts` still sets in-process gauges a CronJob pod can never have scraped | FUP-2026-09-22-sync-metrics | 🟡 (the first is the deployment repository's) · **shape decided 2026-09-24, not yet built**: the last **full** pass's drift gets its own column and its own gauge **`prisme_sync_drift_full_objects`**, so §5's *"above 0 on two consecutive daily full passes"* is expressible exactly, as `min_over_time(prisme_sync_drift_full_objects[48h]) > 0`. Additive — a new column, a new metric, a new row in the spec's table. **Labelling the existing gauge was rejected**, because a labelled series only carries samples in the minutes after a full pass, so the expression would mean *the last one* rather than *two consecutive*; and changing §5's wording was rejected because the rule is right and only its expression was missing. Deciding this was not implementing it — [the entry](docs/50-journal/FUP-2026-09-24-drift-provenance-decision.md) | — |
