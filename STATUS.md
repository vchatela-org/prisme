...
| **Adopting a task would rewrite it on the first writing pass** — its deadline cleared, its priority reset to the lowest, the task moved to its area's first mapping, each a silent first-time `update` | found writing the user guide (nobody had recorded it) | 🟢 deadline taken into prisme at the link; priority and location left alone until prisme has a decision · [the entry](docs/50-journal/FUP-2026-09-26-adoption-keeps-task-fields.md) | [#95](https://github.com/vchatela-org/prisme/pull/95) |
| **The adoption queue refreshed only when somebody ran a Job, and no takeaway ever reached the Inbox** — the scan had no schedule and no button; the takeaway-type property was never passed and nothing wrote `takeaway` | found writing the user guide (G6, G9, G10) | 🟢 daily scan in the full pass, *Rescan* on `/adoption`, `DOCTOOL_TAKEAWAY_TYPE_PROPERTY`, takeaway mirror · [the entry](docs/50-journal/FUP-2026-09-26-adoption-scan-and-takeaways.md) | [#96](https://github.com/vchatela-org/prisme/pull/96) |
| **Dependencies and conflicts had API writes and no screen** — the initiative screen named dependencies by identifier, and the review's ledger step could not close a conflict | found writing the user guide (G8) | 🟢 dependency editor on the initiative screen; resolve buttons in the review · [the entry](docs/50-journal/FUP-2026-09-26-rituals-dependencies-conflicts.md) | [#97](https://github.com/vchatela-org/prisme/pull/97) |
...
|---|---|---|---|
| `v0.4.0` | 2026-09-26, on `47b3110` | the wave over [#89](https://github.com/vchatela-org/prisme/pull/89)–[#98](https://github.com/vchatela-org/prisme/pull/98): **the screens an instance was missing** — Settings (areas, colours, task-tool locations, document-tool bindings), an adoption path that no longer erases a deadline, a priority or an anchor and a queue that refreshes itself, screens for rituals, dependencies and conflict resolution — and [a user guide](docs/18-user-guide.md) written for the person using prisme | 🟢 both images published, a digest recorded per image. **Not yet pinned** — release, then pin ([the entry](docs/50-journal/FUP-2026-09-26-v040-and-the-screens-an-instance-was-missing.md)) |
| `v0.3.0` | 2026-09-25, on `66b910e` | the follow-up wave over [#84](https://github.com/vchatela-org/prisme/pull/84)–[#88](https://github.com/vchatela-org/prisme/pull/88): an Accepted ADR (0029, *a project has one area*) and the drift alert's series, and **the session that could not end** — an expired login now reaches the login flow instead of a `401` with no way out | 🟢 both images published, digests read back from Harbor, and **pinned** — both tiers run `v0.3.0`, so the expired-session fix is live ([the entry](docs/50-journal/FUP-2026-09-25-v030-and-the-session-that-could-not-end.md)) |
