...
| **The adoption queue refreshed only when somebody ran a Job, and no takeaway ever reached the Inbox** — the scan had no schedule and no button; the takeaway-type property was never passed and nothing wrote `takeaway` | found writing the user guide (G6, G9, G10) | 🟢 daily scan in the full pass, *Rescan* on `/adoption`, `DOCTOOL_TAKEAWAY_TYPE_PROPERTY`, takeaway mirror · [the entry](docs/50-journal/FUP-2026-09-26-adoption-scan-and-takeaways.md) | — |
