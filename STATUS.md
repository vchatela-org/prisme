...
| `AREA_COLOR_PINS` must be set by hand and **nothing generates it**, so an instance that pins nothing keeps the collision — `areaColorCollisions` can report the clash and only the gallery calls it | FUP-2026-09-21-area-colour-pinning | 🟢 the **Areas** screen names the colliding areas and prints the line to set, keeping what is already pinned and dealing only the gaps — proved in a browser both with the variable unset and with the proposed map applied | this branch |
