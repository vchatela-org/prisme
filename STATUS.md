...
| **Adopting a task would rewrite it on the first writing pass** — its deadline cleared, its priority reset to the lowest, the task moved to its area's first mapping, each a silent first-time `update` | found writing the user guide (nobody had recorded it) | 🟢 deadline taken into prisme at the link; priority and location left alone until prisme has a decision · [the entry](docs/50-journal/FUP-2026-09-26-adoption-keeps-task-fields.md) | — |
